...
bool globalEvaluate(nodeType *p, int index);
...
bool evaluate(nodeType* p,int index){
	...
	{	
		switch (p->opr.oper)
	{
		case '=':eqEvaluate(p,index);return 1;break;
		case '>':return gtEvaluate(p, index);break;
		case '<':return ltEvaluate(p, index);break;
		case GTR_EQ:return gtrEvaluate(p, index);break;
		case LESS_EQ:return lteEvaluate(p, index);break;
		case NOT_EQ:return neEvaluate(p, index);break;
		case EQ_EQ:return eqeqEvaluate(p, index);break;
		...
		case ARR_D : arrDeclare(p, index); return 1;break;
		case GLOBAL : globalEvaluate(p,index); return 1;break;
		default:
			break;
	}
	printf("rahile 7 \n");
...
void funcEvaluate(nodeType *p, int index){
	...
	for(int i = 0;i<fun_IND;i++){
		...
		printf("prinf %s\n",func_table[func_count].fun_id[i].name);
		printf("value %s\n",func_table[func_count].fun_id[i].val);
	}
	printf("hey Rahile2\n");
